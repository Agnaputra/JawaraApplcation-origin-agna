...
// Kelas ini mengelola semua state input dan logika validasi UI.

class _BroadcastTambahForm extends StatefulWidget {
  _BroadcastTambahForm({super.key});
...
class _BroadcastTambahFormState extends State<_BroadcastTambahForm> {
  ...
  void _submitForm() {
    ...
    // Logika Simulasi Keberhasilan UI (tanpa API call)
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Broadcast "${_judulController.text}" siap dikirim (UI OK)!'),
  ...
  // Komponen TextFormField dengan gaya minimalis (Reusable Component)
  ...
  // Komponen File Upload (sesuai desain image_84afa1.png)
  Widget _buildFileUploadBox(String title, String subtitle, bool isSelected, {required VoidCallback onSimulateUpload}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(title, style: const TextStyle(fontWeight: FontWeight.w600, color: Color(0xFF454545))),
        ...
        InkWell(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: isSelected ? Colors.green.withOpacity(0.1) : const Color(0xFFF0F0F5),
              border: Border.all(
            ...
            child: Center(
              child: isSelected 
                  ? const Text('File berhasil diunggah! (Simulasi)', style: TextStyle(color: Colors.green, fontWeight: FontWeight.w600))
                  : Text('Upload ${title.split(' ')[0].toLowerCase()}/${title.split(' ')[0].toLowerCase()}', style: TextStyle(color: Colors.grey.shade700)),
            ),
          ),
        ),
        const SizedBox(height: 5),
        const Align(
          ...
          child: Text('Powered by PQRIA', style: TextStyle(fontSize: 10, color: Colors.grey)),
        ),
        const SizedBox(height: 20),
      ],
    );
    ...
    return Form(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          // 2. Isi Broadcast (Deskripsi)
          _buildTextField(_pesanController, 'Isi Broadcast', 'Tulis isi broadcast di sini...', maxLines: 6,
            validator: (value) => (value == null || value.isEmpty) ? 'Isi pesan wajib diisi' : null),
          ...
          _buildFileUploadBox(
            ...
            onSimulateUpload: () {
              setState(() {
                _isPhotoSelected = !_isPhotoSelected; // Toggle state saat diklik
              });
            },
          ),
          ...
          _buildFileUploadBox(
            ...
            onSimulateUpload: () {
              setState(() {
                _isDocSelected = !_isDocSelected; // Toggle state saat diklik
              });
            },
          ),
...
// --- BAGIAN 2: KELAS UTAMA (boradcastTambahPage) ---
// Kelas ini mengatur layout utama.

class boradcastTambahPage extends StatelessWidget {
